...
//for            
            case Etiqueta.FOR:
                ...
                coincidir('(');//coincidimos el inicio del bucle for (
                s = asignar(); //se sustrae la asignacion que se le da a la variable
                x = bool(); //la condicional se guarda en x
                s1= instr();// se revisa si continua el ; o tambien se puede utilizar el coincidir()
                s2 = asignar();// se comprueba si existe incrementa nuestra variable
                coincidir(')');//se conincide el parentecis que termina nuestro bucle for
                s3 = instr();//se compueba el contenido de nuestro for
                nodofor.inic(s, x, s1, s2, s3);
                Instr.Circundante = instrGuardada; // restablece Instr.Circundante
                return nodofor;
                //{int x=3; for(x=2; x<4; x++)}
                //{int x=3; for(x=2; x<4; x=x=1){x=x-1;}}
                /*{
int i; 
int x;
i = 1; 
